...
from my_typist.perception import camera_pose_world, estimate_board, load_keyboard_template
...
class Typist(Node):
    def __init__(self) -> None:
        ...
        self.vision_log = VisionLog(
            str(self.get_parameter("vision_log_path").value)
        )
        ...
        self.key_press_publisher = self.create_publisher(Empty, "/arm/press", QOS_RELIABLE)
        ...
        self.create_subscription(Image, "/camera/image_raw", self.image_callback, QOS_SENSOR)
        self.create_subscription(
            CameraInfo, "/camera/camera_info", self.camera_info_callback, QOS_LATCHED
        )
        self.create_subscription(String, "/sim/launch_key", self.launch_key_callback, QOS_LATCHED)
        self.create_subscription(EpisodeResult, "/sim/result", self.result_callback, QOS_LATCHED)
        self.reset_client = self.create_client(Trigger, "/sim/reset")

        self.control_timer = self.create_timer(DT, self.control_loop)
        self.get_logger().info("S2 typist ready; waiting for camera, joints, and launch key")
    ...
    def launch_key_callback(self, msg: String) -> None:
        ...
        if not 3 <= len(launch_key) <= 6 or any(ch not in KEY_LAYOUT for ch in launch_key):
    ...
    def image_callback(self, msg: Image) -> None:
        ...
        try:
            ...
            if detection.reprojection_error_px > 3.0:
                raise ValueError(
                    f"marker reprojection error is {detection.reprojection_error_px:.2f}px"
                )
            if detection.match_score < 0.20:
                raise ValueError(f"keyboard image match is too weak ({detection.match_score:.2f})")

            rotation_world_camera, translation_world_camera = camera_pose_world(self.q)
            self.board_rotation_world = rotation_world_camera @ detection.rotation_camera_board
            ...
            values = (
                ...
                f"{detection.reprojection_error_px:.3f}; match={detection.match_score:.3f}; "
            ...
            self.vision_log.write(
                ...
                "plan",
                "typing_pose",
                f"q_arm={list(self.plan.arm_joints)}; head={list(self.plan.head_position)}",
            )
    ...
    def control_loop(self) -> None:
        ...
        if active and (
            self.joint_state_watchdog.is_stale(now) or self.camera_watchdog.is_stale(now)
        ):
        ...
        if self.current_state == "moving":
            ...
            velocities = [pid.update(error, DT) for pid, error in zip(self.pids, errors)]
    ...
    def result_callback(self, msg: EpisodeResult) -> None:
        ...
        if msg.exact_match:
            self.get_logger().info(f"SUCCESS: typed {msg.typed!r} in {msg.elapsed:.2f}s")
        else:
            self.get_logger().error(
        ...
        if self.episodes_completed < self.episodes_target:
            if self.reset_client.service_is_ready():
                self.get_logger().info(
                    f"Requesting next seed ({self.episodes_completed}/{self.episodes_target})"
                )
                self.reset_future = self.reset_client.call_async(Trigger.Request())
            else:
                self.get_logger().error("/sim/reset is unavailable; cannot run next episode")
